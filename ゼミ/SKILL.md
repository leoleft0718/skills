...
1. ユーザーの要望から本文スライドの構成（章立て）を決める。
2. `templates/title.md` を表紙として配置（氏名・所属は上記をデフォルトで反映）。
3. `templates/mini-corner.md` でミニコーナースライドを配置。
4. `templates/connection.md` で前回と今回のつながりスライドを配置。
   - ユーザーが前回の内容を指定しない場合はプレースホルダーを残す。
5. `templates/toc-summary.md` で目次・サマリースライドを配置。
6. 本文スライドは `templates/content.md` を雛形に章ごとに書く。
   - 2カラムが必要なら `templates/two-column.md` を使う。
7. 本文を書き終えた後、目次・サマリーの内容を本文と整合させる。
8. 本文スライドのインライン引用番号とフッターの対応をチェックする。
9. 末尾に参考文献一覧スライドを配置する。
10. `paginate` / `_paginate: false` の整合をチェックする。
11. **変更後は必ず PDF エクスポートを実行する**。
12. ユーザーにビルドコマンドを提示する。
